...
function findFirstSingleChar(str) {
  ...
  const result = Array.from(str).find((char) => charCount.get(char) === 1);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  // Build the interval string based on inclusion/exclusion flags
...
function getDigitalRoot(num) {
  ...
  while (sum > 9) {
    // Convert the current sum to an array of digits and calculate the sum
...
function isBracketsBalanced(str) {
  ...
  return Array.from(str).every((char) => {
    if (bracketPairs[char]) {
      ...
      return bracketPairs[lastBracket] === char;
    }
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    throw new Error('Radix must be between 2 and 10 (inclusive).');
  }
  let num1 = num;
  if (num1 === 0) {
    return '0';
...
function getMatrixProduct(m1, m2) {
  ...
  const result = new Array(rowsM1).fill(0).map(() => new Array(colsM2).fill(0));
